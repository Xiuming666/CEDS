# ------------------------------------------------------------------------------
# Program Name: E.Australia_emission_2017Update.R
...
# provide logging, file support, and system functions - and start the script log.
    ...
    script_name <- paste0( em, "-E.Australia_emission_new2017.R" )
...
#   the inventory (as a vector of iso codes)
    inventory_data_file <- paste0( em, '_Australia_UNFCCC_and_NPI_2017Update' )
...
# 2 Inventory in Standard Form (iso-sector-fuel-years, iso-sector-years, etc)
    ...
    # Output a blank df for unsupported species
        ...
        for ( i in seq_along( inv_years ) ) {
            ...
            #select correct columns, rename, and add new year data to total inv. data frame
            keep_columns <- c( "Source", "Total (kg)" )
            ...
            if ( i == 1 ) {
                ...
                #insert new columns if new sectors in current year relative to previous years
                inv_data <- merge( inv_data, inv_singleyear[ 'sector' ], by = 'sector', all = T )
                #find the positions of the current year sectors in the total inv dataset
                sector_pos <- match( inv_singleyear$sector, inv_data$sector)
                # insert the current year data into overall inv data frame based on matching sector name positions
                inv_data$year[sector_pos] <- inv_singleyear[, c(2) ]
                #rename columns
                names( inv_data )[i+1] <- paste0( 'X', inv_years[ i ] )
            }
        }
