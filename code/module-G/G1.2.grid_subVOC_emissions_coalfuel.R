# ------------------------------------------------------------------------------
# Program Name: G1.2.grid_subVOC_emissions_coalfuel.R
# Authors: Erin McDuffie, abapted from Leyang Feng, Caleb Braun
# Date Last Updated: July 4, 2019
# Program Purpose: Grid aggregated emissions into NetCDF grids for subVOCs for coal fuel emissions
#                  Not inlcuding coal, biomass, or aircraft emissions
# Input Files:  MED_OUT:    CEDS_NMVOC_emissions_by_country_CEDS_sector_[CEDS_version].csv
# Output Files: MED_OUT:    gridded-emissions/CEDS_[VOCID]_anthro_[year]_0.5.nc
#                           gridded-emissions/CEDS_[VOCID]_anthro_[year]_0.5.csv
...
initialize( "G1.2.grid_subVOC_emissions_coalfuel.R", log_msg, headers )
...
for ( year in year_list ) {
  ...
  generate_final_grids_nc_subVOC_coalfuel( int_grids_list, output_dir, grid_resolution, year, em = 'NMVOC',
...
checksum_df <- list.files( output_dir, paste0( '_', VOC_em, '_coalfuel.*[.]csv' ), full.names = TRUE ) %>%
  lapply( read.csv ) %>%
  dplyr::bind_rows() %>%
