# ------------------------------------------------------------------------------
# Program Name: G1.2.grid_subVOC_emissions_biofuel.R
# Authors: Erin McDuffie, abapted from Leyang Feng, Caleb Braun
# Date Last Updated: July 4, 2019
# Program Purpose: Grid aggregated emissions into NetCDF grids for subVOCs for biomass fuel emissions
...
initialize( "G1.2.grid_subVOC_emissions_biofuel.R", log_msg, headers )
...
for ( year in year_list ) {
  ...
  generate_final_grids_nc_subVOC_solidbiofuel( int_grids_list, output_dir, grid_resolution, year, em = 'NMVOC',
...
checksum_df <- list.files( output_dir, paste0( '_', VOC_em, '_solidbiofuel.*[.]csv' ), full.names = TRUE ) %>%
  lapply( read.csv ) %>%
  dplyr::bind_rows() %>%
